...
export default function Navbar() {
  ...
  useEffect(() => {
    setRole(getCookie("seam_role") ?? null);
    setReady(true);
  }, []);
      ...
      <div className="mx-auto max-w-[1280px] px-8 h-[72px] grid grid-cols-3 items-center">
        ...
        <div className="flex justify-center">
          <Link href="/" className="flex flex-col items-center">
            <span className="font-display text-[1.65rem] font-bold tracking-[-0.02em] text-[#0f0e0b] leading-none">SEAM</span>
            <span className="text-[0.55rem] tracking-[0.2em] uppercase text-[#6b6757] mt-0.5">Bespoke, Worldwide</span>
          </Link>
        </div>
        ...
        <div className="hidden md:flex items-center justify-end gap-1">
          {ready && role ? (
            <>
              <Link href={dashboardHref} className="text-[0.75rem] font-medium tracking-[0.08em] uppercase text-[#6b6757] px-4 h-[72px] flex items-center hover:text-[#1c1b17] transition-colors">
                Dashboard
              </Link>
              <button onClick={handleSignOut} className="ml-2 border border-[#d0ccbf] text-[#1c1b17] text-[0.72rem] font-semibold tracking-[0.06em] uppercase px-5 py-2.5 rounded-[6px] hover:bg-[#f7f5f0] transition">
                Sign Out
              </button>
            </>
          ) : ready ? (
            <>
              <Link href="/apply" className="text-[0.75rem] font-medium tracking-[0.08em] uppercase text-[#6b6757] px-4 h-[72px] flex items-center border-b-2 border-transparent hover:text-[#1c1b17] transition-colors">
                Apply as Tailor
              </Link>
              <Link href="/auth/login" className="text-[0.75rem] font-medium tracking-[0.08em] uppercase text-[#6b6757] px-4 h-[72px] flex items-center hover:text-[#1c1b17] transition-colors">
                Sign In
              </Link>
              <Link href="/auth/signup" className="ml-2 bg-[#0f0e0b] text-white text-[0.72rem] font-semibold tracking-[0.06em] uppercase px-5 py-2.5 rounded-[6px] hover:opacity-80 transition-opacity">
                Get Started
              </Link>
            </>
          ) : null}
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="bg-white border-[#e6e3da] w-72">
              <div className="mt-8 flex flex-col gap-1">
                {navLinks.map((link) => (
                  <Link key={link.href} href={link.href} onClick={() => setOpen(false)} className="text-sm font-medium tracking-[0.06em] uppercase text-[#6b6757] hover:text-[#1c1b17] py-3 border-b border-[#e6e3da] transition-colors">
                    {link.label}
                  </Link>
                ))}
                {!role && (
