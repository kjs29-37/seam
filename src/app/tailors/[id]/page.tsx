    ...
    <div className="bg-[#fdfcf9] min-h-screen">
      ...
      <div className="border-b border-[#e6e3da] bg-white px-8 py-3">
        <div className="mx-auto max-w-[1280px] flex items-center gap-2 text-[0.75rem] text-[#6b6757]">
          ...
          <span className="text-[#d0ccbf]">›</span>
          <span className="text-[#1c1b17] font-medium">{tailor.studio_name}</span>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="bg-white border border-[#e6e3da] rounded-[6px] p-8 mb-6">
              <div className="flex items-start justify-between gap-6 mb-6">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[0.82rem] text-[#6b6757]">({tailor.review_count} reviews)</span>
                    <span className="text-[#d0ccbf]">·</span>
                    <span className="text-[0.82rem] text-[#6b6757]">Responds {tailor.response_time}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-px bg-[#e6e3da] border border-[#e6e3da] rounded-[4px] overflow-hidden">
                {[
                  { label: "Starting Price", value: `${tailor.currency}${tailor.price_min}` },
                  { label: "Delivery", value: `${tailor.delivery_weeks} weeks` },
                  { label: "Response", value: tailor.response_time },
                ].map((s) => (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-[#e6e3da] border border-[#e6e3da] rounded-[4px] overflow-hidden">
                  {tailor.portfolio.map((item) => (
                    <div key={item.id} className="bg-[#f7f5f0] hover:bg-[#f0ede6] transition-colors">
                      ...
                      <div className="p-3">
                        <p className="text-[0.6rem] font-bold tracking-[0.08em] uppercase text-[#9c9886]">{item.garment_type}</p>
              ...
              {tailor.reviews.length === 0 ? (
                ...
                <div className="space-y-0">
                  {tailor.reviews.map((r, i) => {
                    ...
                    const date = new Date(r.created_at).toLocaleDateString("en-GB", { year: "numeric", month: "long" });
                    return (
                      <div key={r.id} className={`py-5 ${i < tailor.reviews.length - 1 ? "border-b border-[#e6e3da]" : ""}`}>
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-[0.85rem] font-semibold text-[#0f0e0b]">{name}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <StarRating rating={r.overall_rating} />
                            <span className="text-[0.72rem] text-[#9c9886]">{date}</span>
                          </div>
                        </div>
            ...
            <div className="bg-white border border-[#e6e3da] rounded-[6px] p-6">
              ...
              <div className="space-y-0">
                {[
                  ...
                  { label: "Price Range", value: `${tailor.currency}${tailor.price_min} – ${tailor.currency}${tailor.price_max}` },
                  { label: "Production Time", value: `${tailor.delivery_weeks} weeks` },
                  { label: "Response Time", value: tailor.response_time },
                  { label: "Rating", value: `${tailor.rating} ★ (${tailor.review_count} reviews)` },
                  ...(tailor.garment_categories.length > 0 ? [{ label: "Serves", value: tailor.garment_categories.join(", ") }] : []),
                ].map((row) => (
