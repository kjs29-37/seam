          ...
          <div>
            ...
            <div className="bg-white border border-[#e6e3da] rounded-[6px] p-8 mb-6">
              <div className="flex items-start justify-between gap-6 mb-6">
                <div className="h-20 w-20 rounded-[6px] bg-[#f7f5f0] border border-[#e6e3da] flex items-center justify-center text-4xl shrink-0">🧵</div>
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    <h1 className="font-display text-[1.8rem] font-bold text-[#0f0e0b] leading-tight">{tailor.studioName}</h1>
                    {tailor.verified && (
                      <span className="text-[0.6rem] font-bold tracking-[0.06em] uppercase text-[#1a5c38] bg-[#e8f2ec] border border-[#c0d9c8] px-2.5 py-0.5 rounded-full">Verified ✓</span>
                    )}
                  </div>
            ...
            {/* Bio */}
            <div className="bg-white border border-[#e6e3da] rounded-[6px] p-8 mb-6">
              <h2 className="text-[0.65rem] font-bold tracking-[0.12em] uppercase text-[#6b6757] mb-4 pb-3 border-b border-[#e6e3da]">About the Studio</h2>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tailor.specialisms.map((s) => (
                      <span key={s} className="text-[0.72rem] font-medium text-[#6b6757] bg-[#f7f5f0] border border-[#e6e3da] rounded-full px-3 py-1">{s}</span>
                    ))}
                  </div>
                </div>
              )}

              {tailor.garmentCategories.length > 0 && (
                <div className="mt-5">
                  <p className="text-[0.65rem] font-bold tracking-[0.1em] uppercase text-[#6b6757] mb-3">Serves</p>
                  <div className="flex flex-wrap gap-2">
                    {tailor.garmentCategories.map((c) => (
                      <span key={c} className="text-[0.72rem] font-medium text-[#1c1b17] bg-white border border-[#d0ccbf] rounded-full px-3 py-1">{c}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            {tailor.portfolio.length > 0 && (
              <div className="bg-white border border-[#e6e3da] rounded-[6px] p-8 mb-6">
                <h2 className="text-[0.65rem] font-bold tracking-[0.12em] uppercase text-[#6b6757] mb-4 pb-3 border-b border-[#e6e3da]">Portfolio</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-[#e6e3da] border border-[#e6e3da] rounded-[4px] overflow-hidden">
                  {tailor.portfolio.map((item) => (
                    <div key={item.id} className="bg-[#f7f5f0] hover:bg-[#f0ede6] transition-colors">
                      <div className="aspect-square flex items-center justify-center text-4xl border-b border-[#e6e3da]">{item.emoji}</div>
                      <div className="p-3">
            ...
            <div className="bg-white border border-[#e6e3da] rounded-[6px] p-8">
              ...
              {tailor.reviews.length === 0 ? (
                <p className="text-[0.85rem] text-[#9c9886]">No reviews yet.</p>
              ) : (
          ...
          <div className="lg:sticky lg:top-[100px]">
            <div className="bg-white border border-[#e6e3da] rounded-[6px] overflow-hidden mb-4">
              ...
              <div className="p-6 space-y-3">
                <Link href={`/enquiry/new?tailor=${tailor.id}`} className="flex items-center justify-center w-full bg-[#0f0e0b] text-white text-[0.75rem] font-semibold tracking-[0.06em] uppercase py-3.5 rounded-[6px] hover:opacity-80 transition-opacity">
                  Request a Quote
                </Link>
                <Link href={`/enquiry/new?tailor=${tailor.id}`} className="flex items-center justify-center w-full border border-[#d0ccbf] text-[#1c1b17] text-[0.75rem] font-semibold tracking-[0.06em] uppercase py-3.5 rounded-[6px] hover:bg-[#f7f5f0] transition">
                  Message / Enquire
                </Link>
              </div>
              <div className="px-6 pb-5">
                <div className="bg-[#f7f5f0] border border-[#e6e3da] rounded-[4px] p-4 text-[0.78rem] text-[#6b6757] leading-[1.6]">
                  <span className="font-semibold text-[#1a5c38]">🔒 Escrow protected.</span> Your payment is held securely by SEAM and only released after delivery.
                </div>
              </div>
            </div>

            <div className="bg-white border border-[#e6e3da] rounded-[6px] p-6">
              <h3 className="text-[0.65rem] font-bold tracking-[0.12em] uppercase text-[#6b6757] mb-4 pb-3 border-b border-[#e6e3da]">Studio Details</h3>
              <div className="space-y-0">
                {[
                  ...
                  ...(tailor.garmentCategories.length > 0 ? [{ label: "Serves", value: tailor.garmentCategories.join(", ") }] : []),
                ].map((row) => (
