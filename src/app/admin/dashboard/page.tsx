        ...
        <section>
          ...
          <div className="space-y-3">
            {pendingTailors.map((t) => (
              <div key={t.id} className="bg-white border border-[#e6e3da] rounded-[6px] p-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <p className="font-display text-[1rem] font-bold text-[#0f0e0b]">{t.name}</p>
                    <p className="text-[0.78rem] text-[#6b6757] mt-0.5">{t.location} · Applied {t.applied} · {t.id}</p>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <button className="bg-[#1a5c38] text-white text-[0.72rem] font-semibold tracking-[0.06em] uppercase px-4 py-2 rounded-[6px] hover:opacity-80 transition-opacity">
                      Approve
                    </button>
                    <button className="border border-red-200 text-red-700 text-[0.72rem] font-semibold tracking-[0.06em] uppercase px-4 py-2 rounded-[6px] hover:bg-red-50 transition">
                      Reject
                    </button>
                    <button className="border border-[#d0ccbf] text-[#6b6757] text-[0.72rem] font-semibold tracking-[0.06em] uppercase px-4 py-2 rounded-[6px] hover:bg-[#f7f5f0] transition">
                      View Application
                    </button>
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[0.72rem] text-[#9c9886]">{d.id} · {d.orderId} · Raised {d.raised}</span>
                      </div>
                      <p className="font-display text-[1rem] font-bold text-[#0f0e0b]">{d.issue}</p>
                      <p className="text-[0.78rem] text-[#6b6757] mt-0.5">{d.customer} vs {d.tailor}</p>
                    </div>
                    <p className="font-display text-[1.4rem] font-bold text-[#0f0e0b]">{d.amount}</p>
                  </div>
